...
import { Loader2, Plus, Building2, Mail, Trash2, Users as UsersIcon, TrendingUp, ArrowUpDown } from "lucide-react";
    ...
    >
      <div className="flex justify-between items-end shrink-0">
        <div>
          ...
          <p className="text-xs text-muted-foreground uppercase tracking-widest">Entity Relationships & Revenue Intelligence</p>
        </div>
        <div className="flex items-center gap-2">
          <Select value={sortBy} onValueChange={(v) => setSortBy(v as SortBy)}>
            ...
            <SelectContent className="bg-black/90 border-white/10 backdrop-blur-xl">
              ...
              <SelectItem value="value_tier" className="text-xs">By Value Tier</SelectItem>
      ...
      <div className="flex-1 glass-card rounded-lg overflow-hidden flex flex-col border border-white/5">
        <div className="grid grid-cols-[auto_1fr_1.5fr_2fr_auto_auto_auto] gap-4 p-3 bg-black/60 border-b border-white/5 text-[10px] text-muted-foreground uppercase tracking-widest shrink-0">
          <div className="w-8 flex justify-center">ID</div>
          <div>Entity</div>
          <div>Organization</div>
          <div>Comm Link</div>
          <div className="w-20">Status</div>
          <div className="w-28 flex items-center gap-1"><TrendingUp className="w-3 h-3 text-amber-400" /> Revenue</div>
                ...
                return (
                  <div key={client.id} className="grid grid-cols-[auto_1fr_1.5fr_2fr_auto_auto_auto] gap-4 p-3 items-center hover:bg-white/5 transition-colors group">
                    ...
                    <div>
                      ...
                      <span className={`mt-0.5 inline-block px-1.5 py-px rounded border text-[8px] uppercase tracking-widest font-mono ${tierCfg.cls}`}>
                        {tierCfg.label}
                      </span>
                    </div>
                      ...
                      {client.lifetimeValue > 0 ? (
                        <div>
                          ...
                          <div className="text-[9px] text-muted-foreground uppercase tracking-widest">
                            {client.paidInvoiceCount} invoice{client.paidInvoiceCount !== 1 ? 's' : ''}
                          </div>
                        </div>
                      ) : (
                        <span className="text-[10px] text-muted-foreground/30 uppercase tracking-widest">—</span>
                      )}
                    </div>
